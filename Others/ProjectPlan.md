# 作成するゲームの企画書
* ビジュアルプログラミングでC言語を学べる初習者向けのプログラミングゲーム
  オブジェクトを組み合わせて完成させるとちゃんとC言語のテキストとして成立するように+それを上から順に読めるようにした形が理想？
...
　その世界で困っている人や街をプログラムの知識を使って救う→たくさんのバグで世界がうまく動いていない\
  ...
  →ここにゲーム中のストーリーを展開\
    ...
    ・住人が隣に壊れてない橋があるのに，手前の壊れてる橋で立ち往生し大渋滞．IFを使って条件分岐を作成して渋滞解消．\
    ・文字列を扱うプログラムでstrcmpをつかって合言葉を判定する→金庫が正しい番号を入れても開かなくなってしまったのを直す？\
    ・今度は金庫のパスワードを忘れてしまった．whileで総当たりするシステムを作ってパスワードを当てる\
...
■ 新たに追加できそうなエピソード・機能案\
1. 配列の操作（int型やchar型）\
問題：村の子どもたちが順番に名前を名乗っているが、順番がバラバラで誰が何番目か分からなくなってしまった。\
解決：配列を使って名前を順番に保存し、forループで表示順を整える。\
2. 関数（function）の導入\
問題：ある村では何かを処理するたびに全く同じ長い手順を毎回書いているため、作業が効率悪くなっている。\
解決：その手順を関数化して短くまとめてあげる。\
3. 構造体（struct）の活用\
問題：動物園の動物たちの「名前・種類・年齢」がごちゃごちゃになって、どれがどの動物かわからなくなった。\
解決：構造体を作って動物の情報を一括で管理する。\
4. switch文の使用\
問題：自動販売機が動かなくなっている。飲み物の番号を押しても毎回「エラー」しか出ない。\
解決：switch文で飲み物の番号によって適切な商品を返すように修正。\
5. ポインタの導入（中級〜）\
問題：記憶を失った住人が「誰かの記憶」と繋がってしまっている（メモリの混乱）。\
解決：ポインタを使って正しい記憶（変数）にアクセスできるように導く。\
6. 再帰（recursion）の使用\
問題：塔の階段が謎の構造で、上ってもまた最初に戻されてしまう。正しい順に上ると一気に上までたどり着けるという。\
解決：再帰関数を使って階段を探索し、条件に合う道を見つける。\
7. 2次元配列と迷路（マップ探索）\
問題：森が迷路のようになっており、抜け出せなくなってしまった。マップデータをもとに最短ルートを導きたい。\
解決：2次元配列を使って地図を扱い、探索（DFSやBFS）で道を見つける。\
8. ファイルの読み書き（入出力）\
問題：図書館の本のデータがすべて消えてしまった。記録用のファイルが壊れていた。\
解決：正しい形式でファイルに情報を保存・読み込むプログラムを作る。\
9. タイマー・時間計測\
問題：カウントダウンで動く橋が、時間内に渡らないと消えてしまう。村人は何秒かかるか分からず困っている。\
解決：処理時間を測る関数（clock()など）を使って、時間内に橋を渡る方法をシミュレート。\
10. ランダム（乱数）の使用\
問題：宝箱の鍵は毎回異なるコードで開く仕組み。誰も開けられない。\
解決：乱数を使った鍵生成システムを理解し、対応する入力を作成する。\
